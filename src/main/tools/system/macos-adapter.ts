...
export class MacOSAdapter {
  ...
  async pressKeys(keys: string[]): Promise<void> {
    ...
    const useKeyCode = primary === "return" || primary === "tab" || primary === "space" || primary === "escape";
    ...
    const command = useKeyCode
      ? `tell application "System Events" to key code ${escapedPrimary}${using}`
      : `tell application "System Events" to keystroke "${escapedPrimary}"${using}`;
