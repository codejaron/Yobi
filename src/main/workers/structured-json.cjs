...
async function generateStructuredJson(input) {
  ...
  const runGenerateText = input.generateTextFn ?? (await import('ai')).generateText;
  let lastError = null;
...
function buildAttemptPrompt(input) {
  ...
  const responsePreview = String(input.lastResponseText || '').trim();
  if (responsePreview) {
    retrySections.push('');
...
function findBalancedJsonSlice(text, startIndex) {
  const stack = [];
  let inString = false;
  let escaped = false;
...
function stripUnknownKeys(value, error) {
