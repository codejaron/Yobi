...
- **Provider + 模型路由**：内置 OpenAI、Anthropic、OpenRouter、DeepSeek、Qwen、Moonshot、Zhipu、MiniMax，也支持任意 OpenAI-compatible Provider。
...
> 支持先拉取远端模型列表再选择；如果列表为空或拉取失败，也可以继续手动输入模型名。
