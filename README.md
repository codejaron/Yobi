...
从代码现状看，Yobi 的核心定位是：**一个带桌面形态、长期记忆和主动行为能力的多通道桌面 AI 陪伴系统**。
...
- **长期记忆不是黑盒**：Yobi 把 `SOUL / STATE / PROFILE / FACTS / EPISODES` 拆开管理，既能长期记住你，也能让开发者和用户看见、维护和调整记忆结构。
...
- **更像“陪伴操作系统”，而不是单点功能产品**：传统陪伴产品更强调角色设定、情绪价值和消费级互动体验；Yobi 更强调长期关系、真实渠道接入、工具协作和桌面工作流融合。

一句话概括：**Yobi 不是一个把“聊天”做得更像人的陪伴应用，而是一个把“陪伴”做进桌面和日常工作流的桌面 AI 陪伴系统。**
...
- **长期记忆系统**：包含 `SOUL / STATE / PROFILE / FACTS / EPISODES`。
- **Memory 页面**：可编辑 `SOUL`，查看快照，手动触发内核 Tick / 每日任务，以及重置局部记忆。
...
- **Bilibili 素材同步**：当前“浏览同步”实现聚焦 B 站，可扫码登录或手动填 Cookie，支持立即同步与少量自动关注新 UP。

### 工具、技能与扩展

- **内置工具开关**：浏览器工具、系统工具、文件工具、Exa 搜索都可单独启停。
- **工具安全控制**：系统命令支持白名单 / 阻止规则 / 高风险审批；文件工具支持读写分级与路径白名单。
- **定时任务**：支持一次性任务和 Cron；动作可为提醒，或定时执行工具。
...
- `定时任务`：提醒 / 工具调度的创建、编辑、暂停、立即执行、运行记录
- `Skills`：本地技能的导入、启停、预览、删除
- `Provider`：Provider 管理与模型路由配置
- `记忆`：SOUL / STATE / PROFILE / FACTS / EPISODES 快照与维护
